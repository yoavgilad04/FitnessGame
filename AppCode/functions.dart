...
//--------------------------------Disconnection alert---------------------------------//
  Future<void> showAlert(Widget setting, BuildContext context) async
  {
...
{
  if(BLE_Connection().read_value[0] == 666)
  {
    return showDialog<void>(
        context: context, 
        barrierDismissible: false,
        builder: (BuildContext context) {
          return AlertDialog(
            title: const Icon(Icons.error,size: 70, color: Colors.red,),
            content: Text("One or more of the devices might lost connection, Please check the buttons",
            style: Theme.of(context).textTheme.bodySmall!.copyWith(fontSize: 25),
            textAlign: TextAlign.center,),
            actions: [
              Align(
